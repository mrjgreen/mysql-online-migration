...
class IntegrationTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testItMigratesATable()
    {
        ...
        $migrate = new Migrate($connSource, $connDest);

        $file = new \SplFileInfo(__DIR__ . '/int_test.dat');

        @unlink($file);

        $sourceTable = new DatabaseTable('mysql_migrate_int_test', 'customers');
        $destTable = new DatabaseTable('mysql_migrate_int_test', 'customers_new');

        $afterTransfer = function() use($connSource, $sourceTable)
        {
            $table = $sourceTable->getFQName();
...
values (11161,'Technics Stores Inc.','Hashimoto','Juri','6505556809','9408 Furth Circle',NULL,'Burlingame','CA','94217','USA',1165,84600)");
        ...
        $this->assertEquals(
            $connSource->query("SELECT * FROM " . $sourceTable->getFQName())->fetchAll(),
            $connDest->query("SELECT * FROM " . $destTable->getFQName())->fetchAll()
        );
    }
