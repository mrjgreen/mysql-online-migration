...
class IntegrationTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $this->sourceDb = DbConnection::makePdoConnection('mysql:host=localhost', 'root', 'password');
        $this->destDb = DbConnection::makePdoConnection('mysql:host=localhost', 'root', 'password');

        $this->createTestDatabases('mysql_migrate_int_test', 'customers');
    }
    ...
    public function testItMigratesATable()
    {
        ...
        $eventDispatcher->addListener('migrate.transfer.initial', function() use($connSource, $sourceTable)
        {
            ...
            $connSource->query("DELETE FROM $table WHERE customerNumber = ?", array(11124));

            $connSource->query("UPDATE $table SET customerName = ? WHERE customerNumber = ?", array('Joe', 11124));
...
values (11161,'Technics Stores Inc.','Hashimoto','Juri','6505556809','9408 Furth Circle',NULL,'Burlingame','CA','94217','USA',1165,84600)");

            $connSource->query("delete from $table where customerNumber = 11161");

            $connSource->query("update $table SET customerName = 'Joe' where customerNumber = 11161");

            $connSource->query("insert  into $table (`customerNumber`,`customerName`,`contactLastName`,`contactFirstName`,`phone`,`addressLine1`,`addressLine2`,`city`,`state`,`postalCode`,`country`,`salesRepEmployeeNumber`,`creditLimit`)
values (11161,'Technics Stores Inc.','Hashimoto','Juri','1234567','9408 Other Place',NULL,'Burlingame','CA','94217','USA',1165,84600)");

            $connSource->query("update $table SET customerName = 'Jon' where customerNumber = 11161");
...
values (11103,'Atelier graphique','Schmitt','Carine ','40.32.2555','54, rue Royale',NULL,'Nantes',NULL,'44000','France',1370,21000)");
        ...
        $this->assertEquals(
            ...
            $connDest->query("SELECT * FROM " . $destTable->getQualifiedName())->fetchAll()
        );
    }

    private function createTestDatabases($dbName, $tableName)
    {
            $this->destDb->query("DROP DATABASE IF EXISTS $dbName");
            $this->destDb->query("CREATE DATABASE $dbName");
            $this->sourceDb->query("CREATE TABLE $dbName.$tableName (`customerNumber` int(11) NOT NULL,
...
(11148,'Dragon Souveniers, Ltd.','Natividad','Eric','+65 221 7555','Bronz Sok.','Bronz Apt. 3/6 Tesvikiye','Singapore',NULL,'079903','Singapore',1621,103800),
(11151,'Muscle Machine Inc','Young','Jeff','2125557413','4092 Furth Circle','Suite 400','NYC','NY','10022','USA',1286,138500),
(11157,'Diecast Classics Inc.','Leong','Kelvin','2155551555','7586 Pompton St.',NULL,'Allentown','PA','70267','USA',1216,100600),
(11166,'Handji Gifts& Co','Victorino','Wendy','+65 224 1555','106 Linden Road Sandown','2nd Floor','Singapore',NULL,'069045','Singapore',1612,97900);");
    }
}